...
class TempTable
{
    ...
    function insertDataIntoTempTable($data){
        $sql = "INSERT into  " . $this->tempTableName . "
                              (issueTitle,sectionTitle,sectionAbbrev,authors,articleTitle,`year`,datePublished,volume,issue,startPage,endPage,articleAbstract,galleyLabel,
                              authorEmail,fileName,keywords) 
                                values (:issueTitle,:sectionTitle,:sectionAbbrev,:authors,:articleTitle,:year,:datePublished,
        ...
        $this->db->bind(':sectionAbbrev',$data[1]);
        $this->db->bind(':authors',$data[2]);
        $this->db->bind(':articleTitle',$data[3]);
        $this->db->bind(':year',$data[4]);
        $this->db->bind(':datePublished',$data[5]);
        $this->db->bind(':volume',$data[6]);
        $this->db->bind(':issue',$data[7]);
        $this->db->bind(':startPage',$data[8]);
        $this->db->bind(':endPage',$data[9]);
        $this->db->bind(':articleAbstract',$data[10]);
        $this->db->bind(':galleyLabel',$data[11]);
        $this->db->bind(':authorEmail',$data[12]);
        $this->db->bind(':fileName',$data[13]);
